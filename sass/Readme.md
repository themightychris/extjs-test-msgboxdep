...
folder are mapped to the application's root namespace, 'TestModernApp'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "TestModernApp"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'TestModernApp' in the same way
as `"TestModernApp/sass/src"`.
